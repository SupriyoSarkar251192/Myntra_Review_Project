...
class ScrapeReviews:
    ...
    def extract_reviews(self, product_link):
        try:
            ...
            self.product_title = prodRes_html.find("h1", {"class":"pdp-name"}).text

            self.product_rating_value = prodRes_html.find("div", {"class": "index-overallRating"}).div.text
            
            self.product_price = prodRes_html.find("span", {"class": "pdp-price"}).text
    ...
    def extract_products(self, product_reviews: list):
        try:
            ...
            user_rating = [i.text for i in review_html.find_all('div', {'class': 'user-review-starWrapper'})]
            user_comment = [i.text for i in review_html.find_all('div', {'class': 'user-review-reviewTextWrapper'})]
            user_name = [i.find_all('span')[0].text for i in review_html.find_all('div', {'class': 'user-review-left'})]
            user_date = [i.find_all('span')[1].text for i in review_html.find_all('div', {'class': 'user-review-left'})]
